...
<p className="quote">"React doesn't provide a stateful primative simpler than a class component."<p className="author"> - Dan Abramov, React Conf 2018</p></p>
...
```
Passing an empty array tells React to only run this `jsx÷useEffect()` on the **first** render only, exactly the same way you'd use `jsx÷componentDidMount()`. Literally we are saying "only run this if any of the values in the dependancy array change, however the dependancy array is empty therefore no values in it can change, therefore it will only ever run on startup".
...
Again, we set an interval in `jsx÷useEffect()` (with an empty dependancy array so it only runs on the first render), and then we clean it up by `jsx÷return`ing a function from `jsx÷useEffect()` and calling `jsx÷clearInterval()` within it. This is the same as calling `jsx÷componentWillUnmount()` to clear the interval.
...
> **Note**: Even if the function we pass to `jsx÷useEffect` is not invoked due to its dependancy array, `jsx÷useEffect()` is still technically *called* and so wis fine for this purpose; dependancy arrays will not cause an issue in regards to calling hooks in a different order.
