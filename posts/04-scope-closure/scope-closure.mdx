...
> The scope of a variable describes where the variable can be accessed within code. Therefore, the scope of a function describes which variables it has access to. [[2]](https://en.wikipedia.org/wiki/Scope_(computer_science))
...
`jsx÷const`, as its name suggests, defines *constants*. This means that variables defined with `jsx÷const` cannot later be reassigned to a different value (they can still be manipulated e.g pushing a value onto an array declared using `jsx÷const`, however we'll leave that for another day!). [[4]](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/const)
...
### Block Scoping
We can split our code into specific **blocks**. In JavaScript, these are usually always denoted by a set of curly braces `{ }` surrounding some code. [[7]](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/block)
...
If there is no function encapsulating our variable declared with `jsx÷var` (or not block surrounding a variable declared with `jsx÷let` or `jsx÷const`), it is said to be in the **Global** scope. The global scope is visible the *every* function and block we declare in our file. [[9]](https://scotch.io/tutorials/understanding-scope-in-javascript#toc-global-scope)
...
### What is a closure?
A closure is when a function is returned from another function, and **remembers its scope**. [[19]](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Closures) That's little wordy, so let's look at an example. The first part is simple - we must return a function *from* a function.
