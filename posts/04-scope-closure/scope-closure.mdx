...
```
Here we define a variable, `jsx÷a`, within `jsx÷funcOne()`. When we attempt to log `jsx÷a` **within** `jsx÷funcOne()` we can see the value of `jsx÷a`. When we attempt to access `jsx÷a` *outside* of `jsx÷funcOne()`, however, we get a `jsx÷ReferenceError`. That is to say, it seems that `jsx÷a` only exists within `jsx÷funcOne()`, and `jsx÷a` ceases to exist once `jsx÷funcOne()` has finished executing.

To make things a bit more interesting, we could define another variable, `jsx÷b`, in the top level of our code.
...
Even though `jsx÷b` is not defined inside `jsx÷funcOne()`, we can successfully log out the value of `jsx÷b` within it. Because we can *access* variable `jsx÷b` within `jsx÷funcOne()`, we say that `jsx÷b` is within the **scope** of `jsx÷funcOne()`. Contrarily, as we cannot access `jsx÷a` within `jsx÷funcOne()`, we say that `jsx÷a` is **not** within the scope of `jsx÷funcOne()`.

> The scope of a variable describes where the variable can be accessed within code. Therefore, the scope of a function describes which variables it has access to.
...
As you may know, in JavaScript we have three options to declare a new variable: using the `jsx÷let`, `jsx÷const`, or `jsx÷var` keywords.
...
`jsx÷const`, as its name suggests, defines *constants*. This means that variables defined with `jsx÷const` cannot later be reassigned to a different value (they can still be manipulated e.g pushing a value onto an array declared using `jsx÷const`, however we'll leave that for another day!).
...
> Variables declared using `jsx÷let` (and `jsx÷const`) are **block**-scoped, those declared with `jsx÷var` are **function**-scoped.
...
### Block Scoping
We can split our code into specific **blocks**. In JavaScript, these are usually always denoted by a set of curly braces `{ }` surrounding some code.
...
```
When we declare a **block-scoped** variable, it becomes tied to its surrounding block. That is to say, it can only be accessed from within the set of curly braces you declare it in. If we attempt to access a variable in one block that is defined in another, JS will be unable to find it and throw a `ReferenceError` (this is what was happening with `jsx÷a` and `jsx÷b` above!). However, we can *nest* blocks inside each other.
...
```
Even though we are entering a new block (the second `jsx÷if` statement), we are still inside the first block as we haven't left those curly braces that defined block 1. This means, if we define a variable in **block 1**, we can access it in **block 2**.
...
```
To put this in a more relational way, we can imagine this interaction as a conversation between the *block* and the *scope-manager* for that block. When we call a variable within a block, the block asks the scope manager for that block "Heya, they're trying to access a variable called `a`, do we define that anywhere in this block?". The scope-manager then responds "No, we don't - but I'll ask the guy above me", and so they then ask the scope-manager for the next outer block "Heya, we need to know who `a` is referring to, do you define `a` here?". This chain continues until we get to the *global* scope (see below) - at which point if `a` is still not defined then JavaScript throws a `ReferenceError`.

Note, however, that this is a **one-way** interaction. If we declare a variable in block 2, it is inaccessible by block 1 as we have to leave the curly braces that define block 2 to re-enter block 1.
...
> A **block** is defined by a set of curly braces surrounding some code (i.e. a function declaration, a loop, etc). We can *nest* blocks inside other blocks, however this is a one-way street: nested blocks can access their parent blocks, but parent blocks cannot access their nested blocks.
...
If there is no function encapsulating our variable declared with `jsx÷var` (or not block surrounding a variable declared with `jsx÷let` or `jsx÷const`), it is said to be in the **Global** scope. The global scope is visible the *every* function and block we declare in our file.
...
imALittleFunction();
```
There is no block encapulating `jsx÷globalOne` or `jsx÷globalTwo`, and they are tied to the global scope. Therefore, they can be accessed within `jsx÷imALittleFunction()`. While declared within two blocks, `jsx÷globalThree` is also a global variable as it is declared using `jsx÷var`. There is no surrounding function for `jsx÷globalThree` to tie itself to, and so it becomes part of the global scope.
...
```
Here, `jsx÷x` is tied to the block that defines our function, and so is available to the whole function (effectively function scoped). `jsx÷y` is also available to the whole function despite being declared in a nested block, as variables declared with `jsx÷var` are function scoped. 
...
While this may seem like an academic difference between `jsx÷var` and `jsx÷let`/`jsx÷const`, usually `jsx÷let` and `jsx÷const` are preferred. This is because we like to declare variables according to the **Principle of Least Exposure (POLE)**. This means we want a variable to be exposed to the **minimum** number of scopes possible. There are multiple reasons for doing this, however one is because it helps to avoid *name collisions*. This is where we attempt to use the same variable name twice in the same scope (which would lead to it being redefined). If a variable gets defined in the `jsx÷global` scope, then *no* function would be able to use that variable name. Therefore, we usually try to avoid putting variables in the `jsx÷global` scope as much as possible, as this heavily violates the *POLE*. 
...
> Note - this section digs a little into the technicalities of JS under-the-hood: **TL;DR is try to declare all your variables at the top of the scope in which they are used, and don't try to access them before their declaration**.
...
// global scope
```
I just said that **children inherit their parent's scope**. Despite being declared *after* block 2 lexically speaking (as written in code), `jsx÷whatAmI` is *still* defined in block 1, which is block 2's parent scope. Therefore, technically we could argue that we should still have access to `jsx÷whatAmI` in block 2.
...
let whatAmI = "Hello";
```
Here's the same issue, just without many different scopes to worry about. Depending on your JS settings and how you declared `jsx÷whatAmI`, you might see either see `jsx÷undefined` from the `jsx÷console.log`, or an error relating to "accessing `jsx÷whatAmI` before initialization". What you ***won't*** see, however, is a `ReferenceError: whatAmI is not defined`, which is the error that normally occurs when we try and call a variable that doesn't exist within the scope it is accessed from. It seems that, even though we declare the variable `jsx÷whatAmI` **underneath** this `jsx÷console.log(whatAmI)`, the `jsx÷console.log` is still *aware* that `jsx÷whatAmI` exists within the current scope (else, we would see the ReferenceError above).

This is because of a concept in JS called **hoisting**. Before execution, JS does a quick scan through your code in a stage called *lexing/tokenizing*. The details are unimportant (for the current discussion, mind you, we'll definitely look into this another day!), but it just converts your code into a format that the JS engine can understand better **before** it actually executes the code. During this stage, when the JS engine enters a new scope (remember, blocks define new scopes and so this occurs when we enter a new block of code), it will ***move*** all your variable declarations to the top of that scope. **However**, it will only move their *declaration*, not the **values assigned to them**.
...
let whatAmI = "Hello";
```
The above will get roughly converted into the below.
...
whatAmI = "Hello";
```
The `jsx÷let whatAmI;` line is perfectly valid syntax in JS - we don't *have* to include as assignment with a declaration (except when using `jsx÷const`). This means that, by the time the `jsx÷console.log(whatAmI)` occurs, JS is aware that there *will be* a variable called `jsx÷whatAmI`, but for now it is just *undefined*.

**Although it is very important to note, and very **confusing**, that JS is not *actually* saying at the top of the file `jsx÷let whatAmI = undefined`**. Else, we wouldn't see that error about using a variable before its initialization. Nor is it actually saying `jsx÷let whatAmI;`, which would lead to the same thing. All **hoisting** does is tell the JS engine not to freak out when it sees a variable name that will be defined in the future, and allows JS to catch compile time errors.
...
While we can talk about whether variables are block scoped or function scoped, we didn't really answer the question of "what defines a scope?". Well, we just said "its between a set of curly braces", however that's not *entirely* true. In a broader sense, we can talk about a scope being formed **dynamically** or **lexically**. Dynamic scoping means that our variables become bound to a **function execution** - *any* variable defined between when the function is *called* and when it *returns* is within the same scope. To avoid confusion: **JavaScript does not use dynamic scoping**. However, below is an example which could help the conceptualize it.
...
### What is a closure?
A closure is when a function is returned from another function, and **remembers its scope**. That's little wordy, so let's look at an example. The first part is simple - we must return a function *from* a function.
...
Let's walk through this. First we make a call to `jsx÷primitiveUseState()` passing in our `jsx÷initialValue` of and empty string. Next, we assign a variable called `jsx÷useStateVariable` to our `jsx÷initialValue` - this variable is used to keep track of the current value of our stateful variable internally. Next we see our `jsx÷pieceOfState()` function, which returns this internal `jsx÷useStateVariable` which we are using to keep track of the current value of our state. Notice, that we are **closing over** this value, therefore we are able to manipulate it long after `jsx÷primativeUseState()` has finished executing. We also define another function, which merely takes in some `jsx÷newValue` and assigns this to the internal `jsx÷useStateVariable`. Finally, we return and array containing our two functions. **Note** - the `// stuff` refers to other things React is doing in their actual `jsx÷useState` such as error checking and re-rendering our component.
...
## Summary
Here we covered a brief introduction to scope and closures in JavaScript. We defined scope as "where variables can be accessed from", and covered the difference between ***block* scoping** (variables attach to code blocks; `{ }`) and ***function* scoping** (variables attach to the nearest parent function). We also briefly covered **hoisting** and the strange-yet-cool-sounding **temporal dead zone** error which occurs when we attempt to use variables declared with `jsx÷let` or `jsx÷const` before their declaration within the same scope, and how the easiest way to avoid this is simply to define our variables at the top of the function/block which needs them. We explained how JavaScript uses **lexical scoping**, which is when we can define the different scopes purely by how the code is written. We also mentioned that child blocks will inherit the scope of their parent blocks (or functions, if we use `jsx÷var`), and so on until we react the *global* scope. We also briefly looked at **closures** in JavaScript: where we return some inner function from some outer function, where the inner function makes reference to some variable declared in the outer function. Finally, we applied these principles to make a crude implementation of React's `jsx÷useState` hook.
