...
We don't worry about converting JSX into a language that browsers understand (remember those `js÷_react.default.createElement()` calls?), as we use a library called <a href="https://babeljs.io/repl">*Babel*</a> to do this for us (once again for the curious, this process of converting code is called *transpiling*). This is what I used to create those ugly functions above - give it a shot! Take some of the JSX examples shown here or online, and convert them into non-JSX form to see how much easier Babel and React are making it for us! [[11]](https://babeljs.io/repl)
...
obj.inside() // ???
```
What happens when we call `jsx÷obj.inside()`? Well, when we enter a function we enter a new execution context (because `jsx÷functions` are also `jsx÷Objects` in JavaScript), therefore we get a new `this`. A good rule of thumb :
...
If we attempt to execute a a function which is a **reference** to a class method, our `jsx÷this` is `jsx÷undefined`! [[20]](https://www.reddit.com/r/reactjs/comments/5g507t/methods_not_bound_to_es6_classes_why/)
...
will tell our Counter to always use the execution context of the `jsx÷Counter` whenever we call `jsx÷increment`, *regardless of where we call `jsx÷this.increment()`*. But binding `jsx÷this` is a strange behaviour and difficult to grasp, and I'm sure I've said `jsx÷this` enough times for it to lose its meaning by now. The amount of **bolding** and *italics* in those last couple paragraphs should make clear just how easy it is to get confused when talking about the `jsx÷this` object in JS, and how hard it can be to explain. So, we can do option 2 instead:
