...
Well here's where things get a little more finicky. Remember `jsx÷this`? `jsx÷this` refers to the current **execution context**. When we enter a function, we create a **new** execution context, and so our `jsx÷this` refers to the execution context *in which the function was called*. [https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this] A simple way to think of `jsx÷this`:
...
When there is no `jsx÷Object` attached to where we are when we execute our code, `jsx÷this` defaults to be the `jsx÷global` object, which, as it's name suggets, is an object visible to **everything** in our application. This `jsx÷global` object is where functions like `jsx÷setTimeout` are defined, and why we can use them in the top-level of our code. [https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/globalThis] Footnote - having Window, global, globalThis is annoying
...
The last piece of this puzzle, is that in JavaScript, `jsx÷class` methods are not `jsx÷bound` by default. [https://reactjs.org/docs/handling-events.html] Remember when I said that, in a new `jsx÷function`, JavaScript **defaults** to the `jsx÷global` object if there's nothing to the left of the `.`? Well, in JavaScript, **methods** (functions we attach to classes) do **not** bind `jsx÷this` automatically. What does this mean in practice?
...
If we attemt to execute a a function which is a **reference** to a class method, our `jsx÷this` is `jsx÷undefined`! [https://www.reddit.com/r/reactjs/comments/5g507t/methods_not_bound_to_es6_classes_why/]
...
```
 ...
 One important difference, however, is that arrow functions **do not have a `this`**. [https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions] Entering a normal function, we enter a new execution context and therefore have a new `jsx÷this` (which will default to `jsx÷global` or be `jsx÷undefined` if it is a class method). However, when we use `jsx÷this` in an arrow function, JS will look within our function and check "do you define a variable called `jsx÷this`?", and when it doesn't find one it goes up to the next outer function/object/block and checks "do YOU have a `jsx÷this` variable?". This keeps going up the scope chain until a `jsx÷this` is found. This means that the `jsx÷this` in an arrow function is **inherited** from the block/function surrounding it.
...
**BUUUUUUUT** there's one more issue. If you take this code and try and run it, nothing will happen upon clicking the buttons. Nada. Zip. Ziltch. Your IDE might even throw a big red line under your code and tell you *"Don't mutate state directly"*. This goes back to the whole point of why we needed to use a class in the first place: we want to keep track of a variable called `jsx÷count` *and re-render our component while keeping this updated variable and display its new value*. Directly touching `jsx÷this.state` will update the `jsx÷count` variable, but it will **not** re-render the component. Luckily, the smart React guys solved this for us - on that `jsx÷React.Component` class we extend from, they define a method called `jsx÷setState` which we use to mutate the state of our `Counter`. The reason being is that `jsx÷.setState()` will cause a re-render of our Counter **after** our state is updated! [https://reactjs.org/docs/state-and-lifecycle.html] We access it with `jsx÷this.setState()` and pass in the updates we want to make: 
