...
> Each component will go through a series of stages from its inital render to the page, to when it is eventually unmounted from the DOM entirely. These stages make up a component's **lifecycle**. There are **three** main lifecycle stages, each with their own functions that run at each stage (lifecycle methods):
...
## PICTURE OF MOUNTING STAGE HERE DEPICTING THE ABOVE
...
![Figure 4 - After clicking "Inc", the "Count" increases and "Hey, I just updated" is logged.](./next-count.JPG)
